...
function void_use_elim_pass!(linfo::LambdaInfo, sv)
    ...
    not_void_use = function (ex::ANY)
        ...
        elseif (isa(ex, Expr) || isa(ex, GotoNode) || isa(ex, LineNumberNode) ||
                isa(ex, NewvarNode) || isa(ex, Symbol) || isa(ex, GlobalRef) ||
                isa(ex, LabelNode))
...
function meta_elim_pass!(linfo::LambdaInfo, code::Array{Any,1})
    ...
    #         deleted. (i.e. the ones that is not eliminated with
    ...
    #
    #    Functions without `propagate_inbounds` has an implicit `false` on the
    #    `inbounds_stack`
    #
    ...
    # The clearing needs to be propagate up during pop
    ...
    for i in 1:length(code)
        ...
        if head === :boundscheck
            ...
            elseif check_bounds == 1 || length(inbounds_stack) >= 2
                # Not inbounds and at least two level deep, this will never
